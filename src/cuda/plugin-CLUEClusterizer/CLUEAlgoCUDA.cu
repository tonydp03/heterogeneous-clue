...
void CLUEAlgoCUDA::makeClusters(PointsCloud const& host_pc) {
  ...
  kernel_calculate_density<<<gridSize, blockSize, 0, stream_>>>(d_hist.get(), d_points.view(), dc_, host_pc.x.size());
  kernel_calculate_distanceToHigher<<<gridSize, blockSize, 0, stream_>>>(
      d_hist.get(), d_points.view(), outlierDeltaFactor_, dc_, host_pc.x.size());
  kernel_find_clusters<<<gridSize, blockSize, 0, stream_>>>(
      d_seeds.get(), d_followers.get(), d_points.view(), outlierDeltaFactor_, dc_, rhoc_, host_pc.x.size());
  ...
  kernel_assign_clusters<<<gridSize_nseeds, blockSize, 0, stream_>>>(d_seeds.get(), d_followers.get(), d_points.view());
  cudaStreamSynchronize(stream_);
}
