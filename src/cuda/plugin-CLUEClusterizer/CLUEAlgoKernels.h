...
__global__ void kernel_reset_hist(LayerTilesCUDA* d_hist) {
...
__global__ void kernel_reset_followers(cms::cuda::VecArray<int, maxNFollowers>* d_followers, int numberOfPoints) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i < numberOfPoints) {
    d_followers[i].reset();
  }
